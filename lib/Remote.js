...
var Remote = function(ipAddress, options) {
  ...
  this.options = _.extendOwn({
    ...
    pairServer: {
      mdnsName: '0000000000000000000000000000000000000001',
      listenPort: 49152,
      pairingTimeout: 60, // 30 seconds
      pairCode: null, // Should generate this
  ...
  _setAddress(ipAddress,3689);

  var _rp = require('request-promise').defaults({
    baseUrl: me.baseUrl,
  ...
  // Builds the base url
  var _setAddress = function(ip, port) {
    this.ipAddress = ip;
    this.port = port;
      ...
      app.get('/pair', function (req, res) {
        ...
        if (query.hasOwnProperty('pairingcode') && query.hasOwnProperty('servicename')) {
          if (query.pairingcode === expectedHash) {
            ...
            crypto.randomBytes(8, function(ex, randBuf) {
